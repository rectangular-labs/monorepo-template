...
import { Sidebar as SidebarIcon, X } from "@rectangular-labs/ui/components/icons";
...
export function Sidebar({ footer, banner, components, menuItems = [], ...rest }: SidebarProps) {
  return (
    <SidebarContent {...rest}>
...
function SidebarContent({ ref: refProp, className, children, ...props }: ComponentProps<"aside">) {
  ...
  return (
    ...
    <motion.div
      className={cn(
        "fixed inset-0 py-10 z-30 backdrop-blur-md bg-background/60",
