...
import { CaretDown } from "@rectangular-labs/ui/components/icons";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
  ...
  return (
    ...
    >
      <Collapsible
        ...
        data-toc-popover=""
        className={cn("relative h-9 animate-fade-in", className)}
        {...rest}
      >
        <header
          ref={ref}
          className={cn(
            "absolute w-full bottom-0 border rounded-xl transition-colors bg-secondary text-secondary-foreground backdrop-blur-sm",
            open && "shadow-lg bg-popover/80 text-popover-foreground",
          )}
        >
          {children}
        </header>
...
function PageTOCPopoverTrigger({ className, ...props }: ComponentProps<"button">) {
  ...
  const spanProps = {
    ...
    className: cn(open && "text-popover-foreground truncate"),
  };

  return (
    <CollapsibleTrigger
      className={cn(
        "flex w-full h-8.5 items-center text-sm text-muted-foreground gap-2.5 px-2 text-start focus-visible:outline-none [&_svg]:size-4",
        className,
      )}
    ...
    >
      ...
      <CaretDown className={cn("ms-auto shrink-0 transition-transform", open && "rotate-180")} />
    </CollapsibleTrigger>
  );
}
