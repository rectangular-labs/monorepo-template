...
export function TOCItems({ ref, className, thumbBox = true, children, ...props }: TOCItemsProps) {
  ...
  const onPrint = useCallback(() => {
        ...
        output.push(
          <g key={i} transform={`translate(${x}, ${(top + bottom) / 2})`}>
            ...
            <text
              ...
              className="fill-primary-foreground font-medium text-xs leading-none font-mono rtl:-scale-x-100"
            >
              {item._step}
            </text>
          </g>,
    ...
    output.unshift(
      <path key="path" d={d} className="stroke-primary" strokeWidth="1" fill="none" />,
    );
...
function ThumbTrack({ computed, thumbBox }: { computed: ComputedSVG; thumbBox: boolean }) {
  ...
  return (
    ...
    >
      ...
      {thumbBox && (
        <div
          className="absolute left-0 size-1 bg-primary rounded-full [offset-distance:var(--offset-distance,0)] opacity-(--opacity,0) transition-[opacity,offset-distance]"
          style={{
...
export function TOCItem({
        ...
        >
          ...
          {item._step !== undefined && (
            <g transform={`translate(${l1 + 0.5}, ${l1 === l2 ? "3" : "6"})`}>
              <circle cx="0" cy="50%" r="8" className="fill-muted" />
              <text
                x="0"
                y="50%"
                textAnchor="middle"
                alignmentBaseline="central"
                dominantBaseline="middle"
                className="fill-muted-foreground font-medium text-xs leading-none font-mono rtl:-scale-x-100"
              >
                {item._step}
              </text>
            </g>
  ...
  return (
    <Primitive.TOCItem
      ...
      {...props}
      className={cn(
        "prose relative py-1.5 text-sm scroll-m-4 text-muted-foreground hover:text-accent-foreground transition-colors wrap-anywhere data-[active=true]:text-primary",
        isFirst && "pt-0",
        isLast && "pb-0",
        props.className,
      )}
