...
function TocThumb({ computed }: { computed: ComputedData }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute inset-y-0 inset-s-0 bg-primary w-px transition-[clip-path]"
      style={{
...
export function TOCItem({
  ...
  return (
    <Primitive.TOCItem
      ...
      {...props}
      className={cn(
        "prose py-1.5 text-sm text-muted-foreground scroll-m-4 transition-colors wrap-anywhere first:pt-0 last:pb-0 data-[active=true]:text-primary hover:text-accent-foreground",
        item.depth <= 2 && "ps-3",
        item.depth === 3 && "ps-6",
        item.depth >= 4 && "ps-8",
        props.className,
