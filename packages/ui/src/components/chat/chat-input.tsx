  ...
  return (
    ...
    >
      <div
        className={cn(
          variant === "default" &&
            "flex w-full flex-col items-end rounded-2xl border border-input bg-transparent p-2 focus-within:ring-1 focus-within:ring-ring focus-within:outline-none",
...
function ChatInputSubmit({
  ...
  if (loading && onStop) {
    return (
      <Button className={className} onClick={onStop} size={"icon"} variant={"outline"} {...props}>
        <svg
          aria-label="Stop"
          fill="currentColor"
          height="24"
  ...
  const isDisabled =
    typeof context.value !== "string" || context.value.trim().length === 0 || props.disabled;
